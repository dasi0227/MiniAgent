...
import { useAgentStore } from '../router/pinia';
...
const saveRename = (session) => {
    if (!session || editingId.value !== session.id) {
        return;
    }
...
const handleDelete = () => {
    if (targetSessionId.value) {
        agentStore.deleteSession(targetSessionId.value);
    }
    ...
    >
        <div class="mb-[12px] flex items-center gap-[12px]">
            <div class="flex items-center gap-[12px]">
                ...
                <div>
                    <div class="text-[16px] font-bold">Dasi Agent</div>
                    <div class="text-[12px] text-[rgba(231,236,244,0.7)]">执行 · 规划 · 编排</div>
                </div>
            </div>
        </div>
        ...
        >
            ...
            >
                <div class="flex items-center justify-between gap-[10px]" @click="handleSelectSession(session.id)">
                    <div class="min-w-0 flex flex-col">
                        ...
                        <template v-else>
                            <div class="mb-[4px] font-semibold">{{ session.title || '未命名会话' }}</div>
                        </template>
            ...
            <div v-if="sessions.length === 0" class="mt-[12px] text-[13px] text-[rgba(231,236,244,0.7)]">
                暂无会话，点击上方按钮开始
            </div>
        </div>
