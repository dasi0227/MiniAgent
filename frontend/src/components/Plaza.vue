...
const loadMineAgent = async () => {
    try {
        ...
        message.value = normalizeError(error).message || '获取 Agent 失败';
    }
};

const doPublish = async () => {
    if (!publishForm.agentId || !publishForm.plazaTitle.trim()) {
        message.value = '请选择 Agent 并填写标题';
        return;
    }
    try {
    ...
    <section class="grid h-screen grid-rows-[1fr_var(--footer-height)] bg-[var(--bg-page)]">
        <div class="overflow-y-auto py-[24px] pl-[24px] pr-[calc(24px+var(--scrollbar-w))]">
            <div class="mx-auto max-w-[980px] space-y-[16px]">
                <h1 class="text-[24px] font-bold">Agent Plaza</h1>

                <div class="rounded-[14px] border border-[var(--border-color)] bg-white p-[14px] space-y-[10px]">
                    <div class="text-[16px] font-semibold">发布我的 Agent</div>
                ...
                <div class="grid gap-[12px] md:grid-cols-2">
                    <div v-for="item in plazaItems" :key="item.plazaId" class="rounded-[14px] border border-[var(--border-color)] bg-white p-[14px] space-y-[8px]">
                        <div class="text-[16px] font-semibold">{{ item.plazaTitle }}</div>
                        <div class="text-[13px] text-[var(--text-secondary)]">{{ item.plazaDesc }}</div>
                        <div class="text-[12px] text-[var(--text-secondary)]">Agent: {{ item.agentId }}</div>
                        <div class="flex items-center gap-[10px] text-[12px] text-[var(--text-secondary)]">
                            <span>👍 {{ item.likeCount }}</span>
                            <span>⭐ {{ item.favorCount }}</span>
                            <span>💬 {{ item.commentCount }}</span>
                        </div>
                        <div class="flex gap-[8px] pt-[4px]">
                            <button class="rounded-[8px] border border-[var(--border-color)] px-[10px] py-[6px] text-[12px]" @click="doLike(item.plazaId)">点赞</button>
                            <button class="rounded-[8px] border border-[var(--border-color)] px-[10px] py-[6px] text-[12px]" @click="doFavor(item.plazaId)">收藏</button>
                            <button class="rounded-[8px] border border-[var(--border-color)] px-[10px] py-[6px] text-[12px]" @click="doFork(item.plazaId)">Fork</button>
                            <button class="rounded-[8px] border border-[var(--border-color)] px-[10px] py-[6px] text-[12px]" @click="openDetail(item.plazaId)">详情</button>
                        </div>
                    </div>
                </div>
