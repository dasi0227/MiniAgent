...
const error = ref('');
const target = ref(route.query?.type === 'admin' ? 'admin' : 'chat'); // chat | admin
...
const submit = async () => {
    ...
    try {
        ...
        if (target.value === 'admin' && user.role !== 'admin') {
            throw new Error('仅管理员可以进入后台');
        }
        const destination = target.value === 'admin' ? '/admin' : '/chat';
    ...
    <div class="flex h-screen items-center justify-center bg-[radial-gradient(120%_120%_at_20%_20%,#dbeafe_0%,#e2e8f0_40%,#f8fafc_100%)] p-[16px]">
        ...
        >
            <div class="mb-[16px] flex items-center justify-between">
                <div class="text-[22px] font-semibold text-[#0f172a]">登录</div>
